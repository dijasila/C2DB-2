...
def plot_cc(ax):
    ...
    ax.text(1.2 + delta, 2.1725, '$E_{excited}^{reorg}$', verticalalignment='center', horizontalalignment='left')
    ...
    ax.text(-0.6 - delta, 0.1725, '$E_{ground}^{reorg}$', verticalalignment='center', horizontalalignment='right')
    ax.plot(x1, parabola(x1))
    ax.plot(x1 + 0.6, parabola(x1)+2)
    ax.set_xticks([], [])
    ax.set_yticks([0, 2], [0, "$E_{excited}$"])
    # plt.yticks([],[])
    ax.set_xlabel('$Q$')
